...
public abstract class SeleniumFunctions {
	...
	protected void waitForElementAndClick(final WebElement element) throws Exception {
		try {
			int i=0;
			while(!isElementPresent(element) && i<15) {
				Thread.sleep(1000);
				i++;
			};
	...
	protected void goToUrl(final String url) throws InterruptedException {
		driver.get(url);
		Thread.sleep(10000);
	}
