...
public class StepsHomePage {
	...
	@Given("^I click at creat an account link$")
	public void i_click_at_creat_an_account_link() throws Throwable {
		homeInteraction.clickCreateAccountButton();
	}
	...
	public void closeBrowser() throws Exception {
		if (SeleniumTestWrapper.getDriver() != null) {
			...
			String path = new File("target" + separator + "screenshots" + separator
					+ SeleniumTestWrapper.getScenario().getName().replace(" ", "_").replace("-", "_") + "_" + id[3])
							.getAbsolutePath();
			...
			if (SeleniumTestWrapper.getScenario().isFailed()) { // verifica se o cenários não passou
				File destPath = new File(destDir.getAbsolutePath() + System.getProperty("file.separator")
						+ SeleniumTestWrapper.getScenario().getName().replace(" ", "_").replace("-", "_") + "_" + id[3]
	...
	public void logConfiguration(String browserName) throws Exception {
		try {
			...
			String path = new File(pathLogsConf + separator + "log4j_" + nameSc + "_" + id[3] + "_" + browserName + "_"
					+ DateTimeHelper.getCurrentDateTime() + ".properties").getAbsolutePath();

			File file = new File(path);

			Properties properties = new Properties();

			properties.setProperty("log4j.appender.RFILE.File", "target" + separator + "log" + separator + nameSc + "_"
					+ id[3] + "_" + browserName + "_" + DateTimeHelper.getCurrentDateTime() + ".log");
			...
			properties.setProperty("log4j.appender.RFILE.layout.ConversionPattern", "%d{yyyy-MM-dd HH:mm:ss} [%-5p] ["
					+ SeleniumTestWrapper.getScenario().getName() + "][" + id[3] + "][%c{1}] - [%M] %m%n");
			...
			properties.setProperty("log4j.appender.STDOUT.layout.ConversionPattern",
					"%d{yyyy-MM-dd HH:mm:ss} [%-5p][" + browserName + "][" + SeleniumTestWrapper.getScenario().getName()
							+ "][" + id[3] + "] [%c{1}] - [%M] %m%n");
			try {
